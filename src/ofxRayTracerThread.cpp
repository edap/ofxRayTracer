#include "ofxRayTracerThread.h"

ofxRayTracerThread::ofxRayTracerThread(const vector<of3dPrimitive>& _primitives, const vector<ofMaterial>& _materials, const vector<ofLight>& _lights){
...
ofColor ofxRayTracerThread::L_i(const ofxRTRay& ray) const{
...
shared_ptr<ofxRTSurfel> ofxRayTracerThread::findFirstIntersectionWithThePrimitives(const ofxRTRay& ray) const{
...
ofColor ofxRayTracerThread::L_0(const shared_ptr<ofxRTSurfel>& surfelY, const glm::vec3 wo) const{
...
*/
ofFloatColor ofxRayTracerThread::L_scatteredDirect(const shared_ptr<ofxRTSurfel>& surfelX,const glm::vec3 wo) const{
...
bool ofxRayTracerThread::visible(const glm::vec3& P, const glm::vec3& direction, const float& distance) const{
...
glm::vec3 ofxRayTracerThread::getPointOnTriangle(const ofxRTRay& _ray, const glm::vec3& _baryPosition) const {
    return _ray.origin + (_ray.direction * _baryPosition.z);
};

ofFloatColor ofxRayTracerThread::getColor(const ofMeshFace &face, int indexMeshes) const {
